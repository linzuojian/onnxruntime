...
TEST(GroupQueryAttentionTest, WebGPU_GraphCapture_ReplayCrossesLocalWindowBoundary) {
  constexpr int batch_size = 1;
  constexpr int sequence_length = 1;
  constexpr int cache_sequence_length = 130;
  constexpr int capture_total_sequence_length = 32;
  // At 129 tokens the first 64-token KV tile is wholly outside the window. This
  // exercises the neutral-metadata early return in addition to partial-tile masking.
  constexpr int replay_total_sequence_length = 129;
  constexpr int local_window_size = 64;
  constexpr int num_heads = 2;
  constexpr int kv_num_heads = 1;
  constexpr int head_size = 128;
  constexpr int hidden_size = num_heads * head_size;
  ...
  auto make_model = [&]() {
    ...
    auto model = std::make_unique<onnxruntime::Model>(
        "gqa_graph_capture_local_window", true, ModelMetaData(), PathString(),
        IOnnxRuntimeOpSchemaRegistryList(), domain_to_version,
  ...
  const std::string model_data = make_model();
  auto run = [&](bool enable_graph_capture) {
    SessionOptions session_options;
    InferenceSession session{session_options, GetEnvironment()};
    auto webgpu_ep = WebGpuEPForGqaOptions(enable_graph_capture, /*enable_turbo_quant=*/false);
    if (!webgpu_ep) {
    ...
    auto query_data = make_data(hidden_size, 0.01f, 29);
    auto key_data = make_data(kv_hidden_size, 0.02f, 23);
    auto value_data = make_data(kv_hidden_size, 0.03f, 19);
    auto past_key_data = make_data(cache_sequence_length * kv_hidden_size, 0.001f, 31);
    auto past_value_data = make_data(cache_sequence_length * kv_hidden_size, 0.002f, 37);
    // Keep the cache deterministic across the warmup/capture run and the replay run.
    std::copy(key_data.begin(), key_data.end(),
              past_key_data.begin() + (capture_total_sequence_length - 1) * kv_hidden_size);
    std::copy(value_data.begin(), value_data.end(),
              past_value_data.begin() + (capture_total_sequence_length - 1) * kv_hidden_size);
    std::copy(key_data.begin(), key_data.end(),
              past_key_data.begin() + (replay_total_sequence_length - 1) * kv_hidden_size);
    std::copy(value_data.begin(), value_data.end(),
              past_value_data.begin() + (replay_total_sequence_length - 1) * kv_hidden_size);

    const TensorShape query_shape{batch_size, sequence_length, hidden_size};
    const TensorShape kv_shape{batch_size, sequence_length, kv_hidden_size};
    const TensorShape cache_shape{batch_size, kv_num_heads, cache_sequence_length, head_size};
    const TensorShape scalar_shape{1};
    auto query_value = make_gpu_value(query_data.data(), DataTypeImpl::GetType<float>(), query_shape);
    auto key_value = make_gpu_value(key_data.data(), DataTypeImpl::GetType<float>(), kv_shape);
    auto value_value = make_gpu_value(value_data.data(), DataTypeImpl::GetType<float>(), kv_shape);
    auto past_key_value = make_gpu_value(past_key_data.data(), DataTypeImpl::GetType<float>(), cache_shape);
    auto past_value_value = make_gpu_value(past_value_data.data(), DataTypeImpl::GetType<float>(), cache_shape);
    std::vector<int32_t> seqlens_data{capture_total_sequence_length - 1};
    std::vector<int32_t> total_sequence_length_data{capture_total_sequence_length};
    auto seqlens_value = make_gpu_value(seqlens_data.data(), DataTypeImpl::GetType<int32_t>(), scalar_shape);
    OrtValue total_sequence_length_value;
    if (enable_graph_capture) {
      total_sequence_length_value = make_gpu_value(
          total_sequence_length_data.data(), DataTypeImpl::GetType<int32_t>(), scalar_shape);
    } else {
      Tensor::InitOrtValue(DataTypeImpl::GetType<int32_t>(), scalar_shape,
                           total_sequence_length_data.data(), cpu_allocator->Info(),
                           total_sequence_length_value);
    }
    Tensor output_tensor(DataTypeImpl::GetType<float>(), query_shape, gpu_allocator);
    OrtValue output_value;
    Tensor::InitOrtValue(std::move(output_tensor), output_value);
    ...
    seqlens_data[0] = replay_total_sequence_length - 1;
    total_sequence_length_data[0] = replay_total_sequence_length;
    update_gpu_value(seqlens_value, seqlens_data.data(), DataTypeImpl::GetType<int32_t>(), scalar_shape);
    if (enable_graph_capture) {
      update_gpu_value(total_sequence_length_value, total_sequence_length_data.data(),
                       DataTypeImpl::GetType<int32_t>(), scalar_shape);
    }
    ORT_THROW_IF_ERROR(session.Run(run_options, *io_binding));

    const auto& gpu_output = io_binding->GetOutputs()[0].Get<Tensor>();
    Tensor cpu_output(DataTypeImpl::GetType<float>(), gpu_output.Shape(), cpu_allocator);
    ORT_THROW_IF_ERROR(ep->GetDataTransfer()->CopyTensor(gpu_output, cpu_output));
    return std::vector<float>(cpu_output.Data<float>(),
                              cpu_output.Data<float>() + cpu_output.Shape().Size());
  };

  const auto captured_output = run(/*enable_graph_capture=*/true);
  if (captured_output.empty()) {
    GTEST_SKIP() << "WebGPU EP not available";
  }
  const auto eager_output = run(/*enable_graph_capture=*/false);
  ASSERT_EQ(captured_output.size(), eager_output.size());
  for (size_t i = 0; i < captured_output.size(); ++i) {
    EXPECT_NEAR(captured_output[i], eager_output[i], 2e-3f) << "output index " << i;
  }
}
